...
export function UsageProvider({ children }: { children: ReactNode }) {
  ...
  const load = useCallback(async (providerId: string) => {
    ...
    try {
      ...
      // request that never had a session - otherwise the one route that throws
      // people out mid-session is also the one that forgets what they were
      // looking at, and a deep link into a project becomes the overview. The
      // value is read off `location`, so it is same-origin by construction, and
      // the login page refuses anything that is not a same-site path anyway.
